	...
	allPackages := []pkgInfo{}
	{
		...
		packages.Visit(rootPkgs, func(p *packages.Package) bool {
			...
			if p.Module == nil {
				...
				klog.Errorf("Package %s does not have module info. Non go modules projects are no longer supported. For feedback, refer to https://github.com/haproxytech/go-licenses/issues/128.", p.PkgPath)
				return false
			}
...
func (l *Library) FileURL(ctx context.Context, cl *source.Client, filePath string) (string, error) {
	...
	if m.Version == "" {
		...
		// * I have a module at github.com/haproxytech/go-licenses/submod.
		...
		// Examples:
		// * https://github.com/haproxytech/go-licenses/blob/HEAD/LICENSE
	...
	// https://github.com/haproxytech/go-licenses/issues/73#issuecomment-1005587408
